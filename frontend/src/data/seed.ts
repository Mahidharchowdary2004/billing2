...
export interface SeedHistory {
  ...
  nextBillSeq: number;
  nextPoSeq: number;
  nextIdAfterSeed: number;
}

/** Builds a few days of realistic transaction history so the dashboard/reports aren't empty on first load. */
export function buildSeedHistory(products: Product[], parties: B2BParty[], suppliers: Supplier[]): SeedHistory {
  ...
  seedPosPlan.forEach((plan) => {
    ...
    sales.push({ id: nextId(), no: 'POS-' + billSeq++, date, items, subtotal, discount: 0, gst: gstAmt, total: subtotal, mode: plan.mode, customer: '' });
  });
  ...
  return { invoices, sales, purchases, nextInvoiceSeq: invoiceSeq, nextBillSeq: billSeq, nextPoSeq: poSeq, nextIdAfterSeed: idCounter };
}
